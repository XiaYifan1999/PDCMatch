...
class BaseModel:
    ...
    def get_current_learning_rate(self):
        ...
        """
        return [
            optimizer.param_groups[0]["lr"] for optimizer in self.optimizers.values()
        ]
    ...
    def validation(self, dataloader, tb_logger, update=True):
        ...
        for index, data in enumerate(pbar):
            ...
            if "geo_error" in self.metrics:
                ...
                geo_err = self.metrics["geo_error"](
                    dist_x, corr_x, corr_y, p2p, return_mean=False
                )
            ...
            if self.opt.get("visualize", False):
                ...
                if "faces" in data_x:
                    if os.path.isfile("figures/texture.png"):
                        shutil.copy(
                            "figures/texture.png",
                            os.path.join(
                                self.opt["path"]["visualization"], "texture.png"
                            ),
                        )
                    faces_x, faces_y = to_numpy(data_x["faces"]), to_numpy(
                        data_y["faces"]
                    )
                    file_x = os.path.join(
                        self.opt["path"]["visualization"], f"{name_x}.obj"
                    )
                    file_y = os.path.join(
                        self.opt["path"]["visualization"], f"{name_x}-{name_y}.obj"
                    )
                    write_obj_pair(
                        file_x,
                        file_y,
                    ...
                    file_x = os.path.join(
                        self.opt["path"]["visualization"], f"{name_x}.ply"
                    )
                    file_y = os.path.join(
                        self.opt["path"]["visualization"], f"{name_y}-{name_x}.ply"
                    )
                ...
                sio.savemat(
                    os.path.join(
                        self.opt["path"]["visualization"], f"{name_x}-{name_y}.mat"
                    ),
                    save_dict,
                )
        ...
        if len(geo_errors) != 0:
            ...
            if tb_logger is not None:
                ...
                # save pcks
                np.save(
                    os.path.join(self.opt["path"]["visualization"], "pck.npy"), pcks
                )
            ...
            if update and (
                ...
                or (metrics_result["avg_error"] < self.best_metric)
            ):
                self.best_metric = metrics_result["avg_error"]
                self.best_networks_state_dict = self._get_networks_state_dict()
                logger.info(
                    f"Best model is updated, average geodesic error: {self.best_metric:.4f}"
                )
    ...
    def _setup_schedulers(self):
        ...
        for name, optimizer in self.optimizers.items():
            ...
            if scheduler_type in "MultiStepRestartLR":
                self.schedulers[name] = MultiStepRestartLR(
                    optimizer, **scheduler_opts[name]
                )
                ...
                self.schedulers[name] = optim.lr_scheduler.MultiStepLR(
                    optimizer, **scheduler_opts[name]
                )
                ...
                self.schedulers[name] = optim.lr_scheduler.LambdaLR(
            ...
            if self.opt["num_gpu"] > 1:
                if self.opt["backend"] == "ddp":
                    ...
                    network = DataParallel(
                        network, device_ids=list(range(self.opt["num_gpu"]))
                    )
                else:
                    raise ValueError(
                        f'Invalid backend: {self.opt["backend"]}, only supports "dp", "ddp"'
                    )
    ...
    def _get_networks_state_dict(self):
        ...
        state_dict = dict()
        for name in self.networks:
            state_dict[name] = deepcopy(
                self._get_bare_net(self.networks[name]).state_dict()
            )
    ...
    def print_networks(self):
        ...
        for net in self.networks.values():
            if isinstance(net, (DataParallel, DistributedDataParallel)):
                net_cls_str = (
                    f"{net.__class__.__name__} - {net.module.__class__.__name__}"
                )
            else:
    ...
    def resume_model(self, resume_state, net_only=False, verbose=True):
        ...
        for name in self.networks:
            ...
            net_state_dict = {
                k.replace("module.", ""): v for k, v in net_state_dict.items()
            }
        ...
        if not net_only:
            ...
            if verbose:
                logger = get_root_logger()
                logger.info(
                    f"Resuming training from epoch: {self.curr_epoch}, "
                    f"iter: {self.curr_iter}."
                )
