...
@LOSS_REGISTRY.register()
class SquaredHilbertSchmidtLoss(nn.Module):
    def __init__(self, loss_weight=1.0):
        super().__init__()
        self.loss_weight = loss_weight

    def forward(self, a, b, sqrtMk_x, sqrtMk_y):
        invsqrtMk_x = torch.linalg.pinv(sqrtMk_x)
        a = torch.matmul(sqrtMk_y, torch.matmul(a, invsqrtMk_x))
        b = torch.matmul(sqrtMk_y, torch.matmul(b, invsqrtMk_x))
        diff_hs = a - b
        loss = torch.sum(torch.abs(diff_hs) ** 2, dim=(-2, -1))
        return self.loss_weight * torch.mean(loss)
...
class SURFMNetLoss(nn.Module):
    ...
    def forward(self, C12, C21, evals_1, evals_2):
        ...
        return losses
@LOSS_REGISTRY.register()
class HS_SURFMNetLoss(nn.Module):
    """
    Loss as presented in the SURFMNet paper. But only adapted the Orthogonality term for elastic basis.
    Orthogonality + Bijectivity 
    """
    ...
    def __init__(self, w_bij=1.0, w_orth=1.0, w_lap=1e-3):
        ...
        Args:
            w_bij (float, optional): Bijectivity penalty weight. Default 1e3.
            w_orth (float, optional): Orthogonality penalty weight. Default 1e3.
            w_lap (float, optional): Laplacian commutativity penalty weight. Default 1.0. Not used.
        """
        super(HS_SURFMNetLoss, self).__init__()
        assert w_bij >= 0 and w_orth >= 0 
        self.w_bij = w_bij
        self.w_orth = w_orth

    def forward(self, C12, C21, evals_1, evals_2, Mk_1, Mk_2):
        """
        Compute bijectivity loss + orthogonality loss
                            + Laplacian commutativity loss
                            + descriptor preservation via commutativity loss
        ...
        # Orthogonality penalty
        if self.w_orth >= 0:
            C21_adjoint = torch.bmm(torch.inverse(Mk_2), torch.bmm(C21.transpose(1,2), Mk_1))
            C12_adjoint = torch.bmm(torch.inverse(Mk_1), torch.bmm(C12.transpose(1,2), Mk_2))
            orthogonality_loss = criterion(torch.bmm(C12, C12_adjoint), eye_batch) + \
                                 criterion(torch.bmm(C21, C21_adjoint), eye_batch)
            orthogonality_loss *= self.w_orth
            losses['l_orth'] = orthogonality_loss
